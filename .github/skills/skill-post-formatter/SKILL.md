...
所有標題文字（`#`、`##`、`###` 等之後的文字）不應使用粗體標記。

**偵測規則**：搜尋標題行中包含 `**` 的模式，如：
- `## **SRE 的核心概念**`
- `### **案例分析：系統設計**`
- `# **錯誤預算（Error Budget）** 的應用`

**修正方式**：移除標題文字中的 `**` 標記，保留純文字。

| 有問題的寫法 | 修正後 |
|---|---|
| `## **SRE 的核心概念**` | `## SRE 的核心概念` |
| `### **案例分析**` | `### 案例分析` |
| `# **錯誤預算（Error Budget）** 衡量` | `# 錯誤預算（Error Budget）衡量` |

> ⚠️ 只處理標題行（以 `#` 開頭的行），不影響正文段落中的粗體文字。

### 修正 Bold 渲染失敗（CJK 標點邊界問題）

Hugo 使用的 Goldmark 遵循 CommonMark 規範：closing `**` 若**前一個字元為 Unicode 標點**（如 `）`、`」`、`』`），且**後一個字元非標點/空白**（如中文字），則不構成 right-flanking delimiter，bold 不會生效。

**偵測規則**：搜尋符合 `**[text][CJK closing bracket]**[CJK char]` 的模式。

**修正方式**：將結尾的 CJK 括號移到 `**` 之外。

| 有問題的寫法 | 修正後 |
|---|---|
| `**「請求成功率」**來` | `「**請求成功率**」來` |
| `**錯誤預算（Error Budget）**這` | `**錯誤預算**（Error Budget）這` |
| `**延遲（Latency）**非常` | `**延遲**（Latency）非常` |
| `**成本差異「外部化」**的` | `**成本差異「外部化**」的` |

**Opening `**` 在 CJK 開括號之前的問題**：若 `**` 後緊跟 CJK 開括號（如 `「`、`『`），且 `**` 前為一般中文字（非標點/空白），則 opening `**` 不構成 left-flanking delimiter，bold 不會生效。修正方式：將 CJK 開括號移到 `**` 之前。

| 有問題的寫法 | 修正後 |
|---|---|
| `為**「可被量化」的問題**` | `為「**可被量化」的問題**` |
| `是**「重要指標」**的` | `是「**重要指標**」的` |

**Opening `**` 的相同問題**：若 `**` 前為 CJK 括號（如 `）**text**`），opening `**` 也不構成 left-flanking。修正方式：在 `）` 與 `**` 之間加一個半形空格（` `）。

> ⚠️ 僅修正明確失效的 bold 標記，不改動已正常渲染的粗體文字。判斷依據：closing `**` 前是 Unicode 標點 AND 後接非標點/空白字元。
